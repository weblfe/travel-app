...
func (this *UserInfoRepositoryImpl) AdminResetPassword() common.ResponseJson {
	...
	if request.UserId == "" {
		return common.NewInvalidParametersResp("异常请求,确实管理员ID")
	}
	if !this.checkAdminCode(request.UserId, request.Code) {
		return common.NewInvalidParametersResp("非法code")
	}
	// 登陆用户更新自己密码
	if request.Mobile == "" || request.Code == "" {
		return common.NewFailedResp(common.ServiceFailed, "参数缺失,重置密码失败")
	}
	if err := this.resetPasswordByAdminCode(request.Mobile, request.Password); err != nil {
		return common.NewFailedResp(common.ServiceFailed, "重置密码失败", err)
	}
	return common.NewSuccessResp(beego.M{"timestamp": time.Now().Unix(), "tip": "请求重新登陆"}, "更新密码成功")
}

// 检查admin 身份
func (this *UserInfoRepositoryImpl) checkAdminCode(id string, code string) bool {
	uid := os.Getenv("APP_ADMIN_ID")
	if uid == id && uid != "" {
...
// 重置密码
func (this *UserInfoRepositoryImpl) resetPasswordByAdminCode(mobile string, password string) error {
	var (
		user = this.userService.GetByMobile(mobile)
	)
	if user == nil {
