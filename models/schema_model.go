...
func (this *RequestLogModel) getCreateIndexHandler() func(*mgo.Collection) {
	return func(doc *mgo.Collection) {
		...
		// 创建索引
		this.logs(doc.EnsureIndexKey("launchLink"))
		this.logs(doc.EnsureIndexKey("lng", "lat"))
		this.logs(doc.EnsureIndexKey("from", "tags"))
		this.logs(doc.EnsureIndexKey("method", "appId"))
		this.logs(doc.EnsureIndexKey("appVer", "driver"))
		this.logs(doc.EnsureIndexKey("realIp", "remoteAddr"))
	}
}
